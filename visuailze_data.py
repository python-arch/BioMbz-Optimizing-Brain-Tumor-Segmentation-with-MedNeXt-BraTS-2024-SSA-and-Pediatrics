...
def load_data(data_dir, example_id, modalities=["t2f", "t1", "t1c", "t2w"]):
    ...
    for modality in modalities:
        nii_path = os.path.join(data_dir, example_id, f"{example_id}_{modality}.nii.gz") # get the path for each channel
        nii_img = nib.load(nii_path)
        print(nii_img.shape)
        images.append(nii_img.get_fdata()) # get the data
    seg_path = os.path.join(data_dir, example_id, f"{example_id}_seg.nii.gz")
